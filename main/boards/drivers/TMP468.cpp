...
/*
 * Kalibrierung:
 * Aktuell global, da im Projekt nur eine TMP468-Instanz existiert.
 * Kann bei Bedarf instanzlokal gemacht werden.
 */
struct TempCal {
    float scale;
    float off[8];
...
esp_err_t TMP468::init() {
    ...
    // Manufacturer ID prüfen
    ESP_RETURN_ON_ERROR(
        read_reg_16(REG_MAN_ID, &msb, &lsb),
        TAG,
        "Manufacturer ID read failed"
    );
...
float TMP468::get_temperature(int channel) {
    if (channel == 0) return read_local_celsius();
    if (channel < 1 || channel > 8) return NAN;
